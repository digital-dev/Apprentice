// Pure decode/walk logic for UE reflection (UClass/FField/FProperty +
// FName), given already-known GNames/GUObjectArray configuration --
// mirrors cheatVerify.ts's port of resolveMonoTargetAddress: no native
// code, no remote calls, just ReadProcessMemory-shaped reads through a
// caller-supplied callback. See
// docs/superpowers/specs/2026-09-17-ue-reflection-decode-design.md for
// why the calibration numbers below are inputs, not something this
// module discovers itself.

export type ReadBytes = (address: string, length: number) => string | null

export interface FNamePoolConfig {
  gNamesBase: string
...
export interface UObjectArrayConfig {
...
// Matches every other unbounded-walk primitive in this codebase (e.g.
// native/src/scanner.cc's kMaxScanResults) -- a corrupt/misconfigured
...
// branch's ByIndex + FNameEntry::Init decode) -- see the design doc for
// why the numeric config is a required input, never assumed.
...
// each node's raw (undecoded) FName and Offset_Internal. Decoding names is
// the caller's job via decodeFName, keeping this function config-free.
...
// class decodes to "Class" -- the standard "is this actually a UClass,
// not just any object with this name" check. maxObjectsToScan is
// required, not defaulted: a misconfigured arrayConfig makes it easy to
// loop over garbage memory for a long time, and that's a caller decision.
